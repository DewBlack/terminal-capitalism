...
static func build_weekly_charge_log(
	...
	return "[DEBUG][GameManager] gastos cobrados | monto=%s base=%s inactividad=%s notional=%s holdings=%s deuda_actual=%s" % [
...
static func build_intraday_exclusion_log(raw_weekly_notional: float, weekly_notional: float) -> String:
	if raw_weekly_notional <= weekly_notional:
		return ""
	return "[DEBUG][GameManager] notional intradia excluido | bruto=%s efectivo=%s" % [
...
static func build_upgrade_offer_log(
	...
	return "[DEBUG][GameManager] mejoras ofertadas | opciones=%d actividad_tier=%d objetivos=%d | %s" % [
		offered_count,
		activity_tier,
